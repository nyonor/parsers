...
{
	const RENDER_FOR_1C = "renderFor1C";
	const RENDER_FOR_USER = "renderForUser";
	...
	public function GetMinimalPricesOnTires($regionName) {
		...
		foreach($tiresToSearch as $tireToSearch) {
			...
			// та же модель в списке
				...
				//var_dump()
				//var_dump("SAME MODEL... " . $ymModelDetailed->name);

				$this->_db->AddYMModel($ymModelDetailed);
				$ymModelDetailed->cae = $tireToSearch['cae'];

				//var_dump($ymModelDetailed->cae);continue;
	...
	/**
	 * @param $ymModelDetailed YMModelDetailed
	 * @return mixed|YMModel|YMModel[]
	 */
	protected function GetYMModelThroughApiService($ymModelDetailed) {

		$res = null;
		if ($this->_queriesLimit > 0){

			$res = $this->_apiService->FindYMModelsByName($ymModelDetailed);
	...
	protected function RenderForUser() {
		...
		$cols = ['САЕ','Бренд','Модель','Ширина','Профиль','Диаметр','Индекс скорости',
			'Индекс нагрузки', 'Сезон', 'RunFlat', 'Кол-во','Цена', 'Дата обновления цены'];
